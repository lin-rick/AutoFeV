...
MIN_PIXEL_WIDTH =2 #original value: 2
MIN_PIXEL_HEIGHT = 8

MIN_ASPECT_RATIO = 0.02
...
MAX_DIAG_SIZE_MULTIPLE_AWAY = 10

MAX_CHANGE_IN_AREA = 5

MAX_CHANGE_IN_WIDTH = 3
...
MAX_NUMBER_Of_MATCHING_CHARS = 6
...
def loadKNNDataAndTrainKNN():
    ...
    try:
        npaClassifications = np.loadtxt("classifications5.txt", np.float32)                  # read in training classifications
    ...
    try:
        npaFlattenedImages = np.loadtxt("flattened_images5.txt", np.float32)                 # read in training images
    except:                                                                                 # if file could not be opened
...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates:          # for each possible plate, this is a big for loop that takes up most of the function
        ...
        possiblePlate.strChars, possiblePlate.listChar = recognizeCharsInPlate(possiblePlate.imgThresh, longestListOfMatchingCharsInPlate)
...
def findListOfListsOfMatchingChars(listOfPossibleChars):
    ...
    for possibleChar in listOfPossibleChars:                        # for each possible char in the one big list of chars
        ...
        if len(listOfMatchingChars) < MIN_NUMBER_OF_MATCHING_CHARS or len(listOfListsOfMatchingChars) > MAX_NUMBER_Of_MATCHING_CHARS:     # if current possible list of matching chars is not long enough to constitute a possible plate
...
def findListOfMatchingChars(possibleChar, listOfChars):
    ...
    for possibleMatchingChar in listOfChars:                # for each char in big list
        ...
        fltWidthHeightRatio = float(abs(possibleMatchingChar.intBoundingRectWidth)) / float(abs(possibleMatchingChar.intBoundingRectHeight))
...
def recognizeCharsInPlate(imgThresh, listOfMatchingChars):
    ...
    return strChars, listOfMatchingChars
# end function
